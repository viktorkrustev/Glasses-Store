    ...
    private void loadProductDetails() {
        new Thread(() -> {
            ...
            runOnUiThread(() -> {
                ...
                // Проверка за местоположението на изображението
                String imageUrl = product.getImageUrl();

                // Ако изображението е интернет линк
                if (imageUrl != null && (imageUrl.startsWith("http://") || imageUrl.startsWith("https://"))) {
                    Glide.with(this)
                            .load(imageUrl)
                            .placeholder(R.drawable.photo1)  // Местоположение на placeholder изображение
                            .error(R.drawable.photo3)  // Местоположение на изображение при грешка
                            .into(imageView);
                } else {
                    // Ако изображението е локално (например в assets или drawable)
